...
async def verify_receipt(*, user, txn, image_bytes: bytes, mime: str, proof_file: str | None = None, proof_record: dict | None = None) -> dict:
    ...
    rec = {
        ...
        "user_id": user["id"],
        "receipt_storage": (proof_record or {}).get("storage") or ("filesystem" if proof_file else None),
        "receipt_db_id": (proof_record or {}).get("id"),
        "receipt_filename": (proof_record or {}).get("filename"),
        "receipt_mime": (proof_record or {}).get("content_type"),
        "receipt_size_bytes": (proof_record or {}).get("size_bytes"),
        "receipt_file": proof_file,  # legacy only; new uploads use receipt_db_id
