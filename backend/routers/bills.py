...
from core.security import bearer, get_current_user, now_iso, check_pw
...
async def generate_bill(eid: str, body: Optional[GenerateReq] = None, user=Depends(get_current_user)):
    ...
    fee, fee_percent = await payment_service.bill_fee_for_user(user, exp.get("total"))
    fee_paid_via = "wallet"
    new_bal = None
    ...
    if body and body.razorpay_order_id and body.razorpay_payment_id and body.razorpay_signature:
        ...
        # Never trust the frontend fee amount: the Razorpay order used for this
        # bill must equal the server-computed Bill Generation Charges exactly.
        if abs(float(ord_doc.get("amount", 0.0)) - float(fee)) > 0.005:
            raise HTTPException(400, "Fee payment amount does not match this bill")
        if ord_doc.get("bill_expense_id") and ord_doc.get("bill_expense_id") != eid:
            raise HTTPException(409, "This fee payment has already been used for another bill")
        await db.payment_orders.update_one(
            {"order_id": body.razorpay_order_id},
            {"$set": {
                "status": "paid", "payment_id": body.razorpay_payment_id,
                "paid_at": now_iso(), "bill_expense_id": eid,
            }},
        )
        razorpay_ok = True
    ...
    if user.get("user_type") == "corporate" and user.get("company_id"):
        ...
        if razorpay_ok:
            ...
            if fee > 0:
                pin = ((body.wallet_pin if body else None) or "").strip()
                if len(pin) != 4 or not pin.isdigit():
                    raise HTTPException(400, "Enter your 4-digit Wallet PIN")
                if not u.get("wallet_pin_hash"):
    ...
    await db.expenses.update_one({"id": eid}, {"$set": {
        ...
        "bill_fee": fee, "bill_fee_percent": fee_percent,
        "bill_generated_at": now_iso(), "bill_fee_paid_via": fee_paid_via,
    }})
    return {"bill_id": bill_id, "wallet_balance": new_bal, "fee": fee, "fee_percent": fee_percent, "fee_paid_via": fee_paid_via}
