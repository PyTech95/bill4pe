...
from core.security import now_iso, check_pw
...
def _public(txn: dict) -> dict:
    return {
        ...
        "has_screenshot": bool(txn.get("proof_db_id") or txn.get("proof_file")),
...
async def first_scan(user, payee_upi, payee_name=None, merchant_amount=None, expense_draft=None):
    ...
    doc = {
        ...
        "proof_file": None,  # legacy filesystem reference only
        "proof_status": "none",
...
async def submit_proof(
    user, tid, proof_file=None, image_bytes=None, mime=None, proof_record=None
):
    """Attach and verify a payment proof.

    ``proof_record`` is the MongoDB proof metadata used by all new uploads.
    ``proof_file`` remains accepted only for backward compatibility with older
    internal callers/tests; new API traffic never writes a filesystem proof.
    """
    txn = await _owned(user, tid)
    ...
    if not image_bytes or not (proof_record or proof_file):
    ...
    rec = await receipt_verify.verify_receipt(
        user=user, txn=txn, image_bytes=image_bytes, mime=mime,
        proof_record=proof_record, proof_file=proof_file,
    )
    ...
    patch = {
        ...
        "proof_storage": (proof_record or {}).get("storage") or ("filesystem" if proof_file else None),
        "proof_db_id": (proof_record or {}).get("id"),
        "proof_filename": (proof_record or {}).get("filename"),
        "proof_mime": (proof_record or {}).get("content_type"),
        "proof_size_bytes": (proof_record or {}).get("size_bytes"),
        "proof_file": proof_file,  # always None for new MongoDB-backed uploads
...
# ---------------- 5) Wallet PIN + atomic, idempotent fee debit ----------------
async def _verify_wallet_pin(user_id: str, pin: str | None) -> None:
    """Require the user's 4-digit Wallet PIN before any bill-fee wallet debit.

    Verification happens before balance mutation so a missing/wrong PIN can never
    deduct money or generate a bill. The hash is never returned to the client.
    """
    pin = (pin or "").strip()
    if len(pin) != 4 or not pin.isdigit():
        raise ValueError("Enter your 4-digit Wallet PIN")
    u = await db.users.find_one({"id": user_id})
    if not u or not u.get("wallet_pin_hash"):
        raise ValueError("Wallet PIN is not set")
    if not check_pw(pin, u["wallet_pin_hash"]):
...
async def generate_receipt(user, tid, wallet_pin: str | None = None):
