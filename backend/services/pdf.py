...
def _under_thousand(n: int) -> str:
    ...
    rest = n % 100
    return f"{_ONES[n // 100]} Hundred" + (f" {_under_hundred(rest)}" if rest else "")


def _indian_integer_words(n: int) -> str:
    """Indian-numbering words (crore/lakh/thousand) for non-negative integers."""
    if n == 0:
        return "Zero"
    parts = []
    ...
    if crore:
        parts.append(f"{_indian_integer_words(crore)} Crore")
    if lakh:
...
def _amount_in_words(amount) -> str:
    """Render an INR amount as e.g. 'Rupees Fifteen and Fifteen Paise Only'."""
    try:
    ...
    text = f"Rupees {_indian_integer_words(rupees)}"
    if paise:
...
def build_pdf_bytes(expense: dict, user: dict) -> bytes:
    ...
    doc = SimpleDocTemplate(buf, pagesize=A4, topMargin=16 * mm, bottomMargin=16 * mm,
                            leftMargin=16 * mm, rightMargin=16 * mm, title="BILL4PE DIGITAL SELF INVOICE")
    ...
    _doc_title = _snap.get("document_title") or "BILL4PE DIGITAL SELF INVOICE"
    meta = Paragraph(
    ...
    fee_pct = expense.get("bill_fee_percent")
    if fee_pct is None and subtotal > 0 and fee_amt > 0:
        fee_pct = round((fee_amt / subtotal) * 100, 4)
    fee_label = (
        f"Bill Generation Charges (@ {float(fee_pct):g}% of billed amount)"
        if fee_pct is not None else "Bill Generation Charges"
    )

    has_fee_row = show_fee and fee_amt > 0
    if has_fee_row:
        rows.append(["", "", "", "Subtotal", f"{subtotal:.2f}"])
        rows.append(["", fee_label, "", "", f"{fee_amt:.2f}"])
        rows.append(["", "", "", "GRAND TOTAL", f"₹ {grand_total:.2f}"])
    else:
        rows.append(["", "", "", "TOTAL", f"₹ {subtotal:.2f}"])

    amount_words_row = len(rows)
    rows.append([
        Paragraph(f"<b>Amount in Words:</b> {_esc(_amount_in_words(grand_total if has_fee_row else subtotal))}", body_st),
        "", "", "", "",
    ])
    ...
    base_style = [
        ...
        ("BACKGROUND", (0, amount_words_row), (-1, amount_words_row), colors.white),
        ("TOPPADDING", (0, amount_words_row), (-1, amount_words_row), 8),
        ("BOTTOMPADDING", (0, amount_words_row), (-1, amount_words_row), 8),
    ]
    if has_fee_row:
        base_style += [
            ("INNERGRID", (0, 0), (-1, amount_words_row - 3), 0.3, BORDER),
            ("FONTNAME", (3, amount_words_row - 3), (-1, amount_words_row - 3), FONT_B),
            ("BACKGROUND", (3, amount_words_row - 3), (-1, amount_words_row - 3), LIGHT),
            ("ALIGN", (1, amount_words_row - 2), (1, amount_words_row - 2), "LEFT"),
            ("FONTNAME", (1, amount_words_row - 2), (1, amount_words_row - 2), FONT_B),
            ("TEXTCOLOR", (1, amount_words_row - 2), (1, amount_words_row - 2), MUTED),
            ("BACKGROUND", (1, amount_words_row - 2), (-1, amount_words_row - 2), LIGHT),
            # Extra breathing room between Subtotal and Bill Generation Charges.
            ("TOPPADDING", (0, amount_words_row - 2), (-1, amount_words_row - 2), 11),
            ("BOTTOMPADDING", (0, amount_words_row - 2), (-1, amount_words_row - 2), 10),
            ("BOTTOMPADDING", (0, amount_words_row - 3), (-1, amount_words_row - 3), 8),
            ("FONTNAME", (3, amount_words_row - 1), (-1, amount_words_row - 1), FONT_B),
            ("BACKGROUND", (3, amount_words_row - 1), (-1, amount_words_row - 1), LIME),
            ("TEXTCOLOR", (3, amount_words_row - 1), (-1, amount_words_row - 1), NAVY),
        ]
    else:
        base_style += [
            ("INNERGRID", (0, 0), (-1, amount_words_row - 1), 0.3, BORDER),
            ("FONTNAME", (3, amount_words_row - 1), (-1, amount_words_row - 1), FONT_B),
            ("BACKGROUND", (3, amount_words_row - 1), (-1, amount_words_row - 1), LIME),
            ("TEXTCOLOR", (3, amount_words_row - 1), (-1, amount_words_row - 1), NAVY),
        ]
    ...
    # ---- Disclaimer + footer ----
    disclaimer_st = ParagraphStyle(
        "disclaimer", parent=styles["Normal"], fontName=FONT, fontSize=7.5,
        textColor=colors.HexColor("#475569"), leading=10,
    )
    disclaimer = (
        "<b>Disclaimer:</b> Bill4Pe is only a bill generation support service provider. "
        "The bill is generated based on information provided by the customer/user. "
        "Bill4Pe does not verify or guarantee the authenticity, accuracy, completeness, "
        "or legality of the transaction, goods/services supplied, merchant details, or payment made. "
        "Any fraud, misrepresentation, manipulation, or loss arising from the transaction "
        "shall be the sole responsibility of the customer/user, and Bill4Pe shall not be held liable in any manner."
    )
    disclaimer_tbl = Table([[Paragraph(disclaimer, disclaimer_st)]], colWidths=[178 * mm])
    ...
    story.append(Paragraph(
        "<b>Note:</b> This is a system-generated self invoice via BILL4PE. "
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        f"Bill generated by <b>Bill4Pay Global LLP</b> | {_fmt_ist()} | BILL4PE © 2026 | www.bill4pay.com",
        sub_st,
    ))
