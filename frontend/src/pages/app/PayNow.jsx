...
export default function PayNow() {
  ...
  const [feeMethod, setFeeMethod] = useState('wallet');
  ...
  const startPayment = async () => {
    ...
    setBusy(true);
    try {
      let expenseDraft = draft ? { ...draft } : undefined;
      // Location is already requested on the item screen. If the user tapped
      // Pay Now before GPS finished, make one quick final attempt here so the
      // generated receipt does not lose the bill location.
      if (expenseDraft && !expenseDraft.prefill_geo) {
        const geo = await captureCurrentLocation();
        if (geo) {
          expenseDraft = { ...expenseDraft, prefill_geo: geo };
          setDraft(expenseDraft);
          sessionStorage.setItem('bill4pe_draft', JSON.stringify(expenseDraft));
        }
      }
            ...
            <>
              ...
              {isCorporate ? (
                ...
                <div className="rounded-xl border p-4 space-y-4" data-testid="fee-payment-box">
                  <div>
                    <div className="font-semibold text-sm">Pay Bill Generation Charges</div>
                    <p className="text-xs text-muted-foreground mt-1">Choose Wallet or secure online payment. The bill is generated only after the charge is successfully paid.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setFeeMethod('wallet')} className={`rounded-xl border-2 p-3 text-left ${feeMethod === 'wallet' ? 'border-primary bg-primary/5' : 'border-border'}`} data-testid="fee-method-wallet">
                      <Wallet className="h-4 w-4" />
                      <div className="font-semibold text-sm mt-1">Wallet</div>
                      <div className="text-[11px] text-muted-foreground">Balance {money(walletBalance)}</div>
                    </button>
                    <button type="button" onClick={() => setFeeMethod('razorpay')} className={`rounded-xl border-2 p-3 text-left ${feeMethod === 'razorpay' ? 'border-primary bg-primary/5' : 'border-border'}`} data-testid="fee-method-razorpay">
                      <CreditCard className="h-4 w-4" />
                      <div className="font-semibold text-sm mt-1">Online</div>
                      <div className="text-[11px] text-muted-foreground">UPI / PhonePe / Paytm (where available) via Razorpay</div>
                    </button>
                  </div>

                  {feeMethod === 'wallet' ? (
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm"><span className="text-muted-foreground">Amount due</span><span className="font-mono font-semibold">{money(feeDue)}</span></div>
                      <Input
                        ...
                        data-testid="wallet-fee-pin-input"
                      />
                      <Button className="w-full" disabled={busy || !walletOk || walletPin.length !== 4} onClick={() => generate(walletPin)} data-testid="pay-wallet-fee-btn">
                        {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : `Pay ${money(feeDue)} from Wallet & Generate`}
                      </Button>
                      {!walletOk && (
                        <Button variant="outline" className="w-full" onClick={() => nav('/app/wallet')} data-testid="add-money-btn">Recharge Wallet</Button>
                      )}
                      <p className="text-[11px] text-muted-foreground text-center">Wrong PIN = no wallet deduction and no bill generation.</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <Button className="w-full" disabled={busy || !rzpEnabled} onClick={payFeeRazorpay} data-testid="pay-fee-btn">
                        {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : `Pay ${money(feeDue)} Online via Razorpay & Generate`}
                      </Button>
                      {!rzpEnabled && <p className="text-[11px] text-amber-700 text-center">Razorpay is not configured yet. Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in backend/.env.</p>}
                    </div>
                  )}
                </div>
              ))}
