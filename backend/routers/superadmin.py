...
async def stats(_=Depends(require_super_admin)):
    ...
    bills_total = await db.expenses.count_documents({"bill_generated": True})
...
# ---------- Bill-generation fee % (per user type) ----------

class BillFeeUpdate(BaseModel):
    individual: Optional[float] = None
    corporate: Optional[float] = None


@router.get("/bill-fees")
async def get_bill_fees(_=Depends(require_super_admin)):
    from services import payment_service
    return await payment_service.get_bill_fee_percents()


@router.put("/bill-fees")
async def update_bill_fees(body: BillFeeUpdate, _=Depends(require_super_admin)):
    from services import payment_service
    if body.individual is None and body.corporate is None:
        raise HTTPException(400, "Provide individual and/or corporate percent")
    try:
        updated = await payment_service.set_bill_fee_percents(
            individual=body.individual, corporate=body.corporate
        )
...
async def list_users(
    ...
    cursor = db.users.find(
        query, {"_id": 0, "password": 0, "wallet_pin_hash": 0}
