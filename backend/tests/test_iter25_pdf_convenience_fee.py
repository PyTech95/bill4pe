...
CORPORATE (subscription) bills must NOT print a 'Bill Generation Charges' / 'GRAND TOTAL'
line; they print a single 'TOTAL Rs X' row equal to the subtotal.
INDIVIDUAL bills (fee > 0) must still print Subtotal + Bill Generation Charges + GRAND TOTAL.
"""
import io
import os
import time
...
# ---------------- CORPORATE EMPLOYEE PDF: no Bill Generation Charges ----------------
class TestCorporatePdfNoConvenienceFee:
    def test_employee_pdf_has_no_fee_rows(self, s, super_token):
        ...
        assert "Bill Generation Charges" not in txt, "corporate PDF must not show Bill Generation Charges"
    ...
    def test_corporate_admin_pdf_has_no_fee_rows(self, s):
        ...
        assert "Bill Generation Charges" not in txt, txt[-500:]
...
class TestIndividualPdfShowsFee:
    def test_individual_pdf_shows_convenience_fee(self, s, super_token):
        ...
        flat = " ".join(txt.split())
        assert "Bill Generation Charges" in flat, flat[-500:]
